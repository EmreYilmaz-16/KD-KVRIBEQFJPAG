...
<cfif len(attributes.seri_no)>
 ...
 FROM #dsn3#.SERVICE_GUARANTY_NEW WHERE SERIAL_NO='#attributes.seri_no#' 
    </cfquery>
<cfquery name="seriSB" datasource="#dsn3#">
SELECT SHELF_NUMBER,SERIAL_NO,DEPARTMENT_ID,LOCATION_ID,SUM(CASE WHEN IN_OUT =1 THEN 1 ELSE -1 END) AS V FROM #dsn3#.SERVICE_GUARANTY_NEW
...
LEFT JOIN #dsn#.DEPARTMENT AS D ON D.DEPARTMENT_ID = T.DEPARTMENT_ID
LEFT JOIN #dsn#.STOCKS_LOCATION AS SL ON SL.DEPARTMENT_ID = T.DEPARTMENT_ID AND SL.LOCATION_ID = T.LOCATION_ID
WHERE T.V > 0
