...
HAVING sum(sr.STOCK_IN - SR.STOCK_OUT) > 0
	...
	,(SELECT TOP(1) ESR.DELIVER_PAPER_NO  FROM catalystTest_1.EZGI_SHIP_RESULT AS ESR,catalystTest_1.EZGI_SHIP_RESULT_ROW AS ESRR WHERE ESR.SHIP_RESULT_ID=ESRR.SHIP_RESULT_ID AND ORDER_ID=ORD.ORDER_ID) AS DELIVER_PAPER_NO 
	,(SELECT TOP(1) ESR.SHIP_RESULT_ID  FROM catalystTest_1.EZGI_SHIP_RESULT AS ESR,catalystTest_1.EZGI_SHIP_RESULT_ROW AS ESRR WHERE ESR.SHIP_RESULT_ID=ESRR.SHIP_RESULT_ID AND ORDER_ID=ORD.ORDER_ID) AS SSHIP_RESULT_ID
...
<!--- <cfquery name="EzgiDeneme1" datasource="#dsn2#">
	select TOP(10) * from #dsn3#.EZGI_ORDERS_ORDERS_REL
	</cfquery>
	<cfquery name="EzgiDeneme2" datasource="#dsn2#">
	select TOP(10) * from #dsn3#.EZGI_SHIP_RESULT
	</cfquery>
	<cfquery name="EzgiDeneme3" datasource="#dsn2#">
	select TOP(10) * from #dsn3#.EZGI_SHIP_RESULT_ROW
	</cfquery>
	<cfquery name="EzgiDeneme4" datasource="#dsn2#">
	select TOP(10) * from #dsn3#.EZGI_SHIPPING_PACKAGE_LIST
	</cfquery>
	<cfdump var="#EzgiDeneme1#">
	<cfdump var="#EzgiDeneme2#">
	<cfdump var="#EzgiDeneme3#">
	<cfdump var="#EzgiDeneme4#">
	<cfdump var="#Sorgu#" > --->
...
<cfoutput>
<a href="javascript://" onclick="windowopen('#request.self#?fuseaction=sales.popup_upd_ezgi_shipping_Depo&ID=1','page');"class="btn btn-primary" style="color:white" title="Sevk Fişine Git">Depo Detay Bilgileri</a>
</cfoutput>
</div></div>
<cf_big_list>
	<tr>
...
<a href="javascript://" onclick="windowopen('#request.self#?fuseaction=sales.popup_upd_ezgi_shipping&iid=#Sorgu.SSHIP_RESULT_ID#','page');" class="tableyazi" title="Sevk Fişine Git">#Sorgu.DELIVER_PAPER_NO#</a>
