...
<cfif session.ep.company_id eq 1>
 	<cfset ezgi_department_id = 26> <!---Firmaya Göre Değişir---> <!---Bursa Depo--->
 	<cfset ezgi_position_type = 19> <!---Firmaya Göre Değişir---> <!---Bursa Depo PDA Personeli--->
<cfelseif session.ep.company_id eq 2>
	<cfset ezgi_department_id = 35> <!---Firmaya Göre Değişir---> <!---İkitelli Depo--->
 	<cfset ezgi_position_type = 52> <!---Firmaya Göre Değişir---> <!---İkitelli Depo PDA Personeli--->
</cfif>
...
<cfquery name="get_emp" datasource="#dsn#">
	...
	FROM            
    	EMPLOYEE_POSITIONS
	WHERE        
    	POSITION_CAT_ID = #ezgi_position_type# AND 
...
<cfelse>
	<cfset attributes.start_date = DateAdd('d',-2,wrk_get_today())>
</cfif>
...
<cfquery name="GET_SHIPPING" datasource="#dsn3#"><!---Sevk Planları ve Sevk Talepleri Listeleniyor--->
            ...
            WHERE
                AMOUNT > 0 AND
                DURUM = 1
                AND (SEVK_EMP IS NULL OR SEVK_EMP=0)
      		ORDER BY
            	SEVK_EMP,
                SEVK_EMIR_DATE	
</cfquery>
...
<cfif GET_SHIPPING.recordcount>
	...
	<cfoutput query="GET_SHIPPING">
                               ...
                               <!----  <cfif session.ep.USERID eq 325>
                              <div style="color:red;font-size:10pt;border-bottom:solid 3px black">
                                                                        SHIP_RESULT_ID :  <cfdump  var="#GET_SHIPPING.SHIP_RESULT_ID#"><br>
                                                                        TİP: <cfdump  var="#IS_TYPE#"> <br>
                                                                        PAKET_SAYISI: <cfdump  var="#AMBAR_CONTROL.PAKET_SAYISI#"> <br>      
                                                                        CONTROL_AMOUNT: <cfdump  var="#AMBAR_CONTROL.CONTROL_AMOUNT#"> <br>    
                                                                        DURUM: <cfdump  var="#GET_SHIPPING.DURUM#"> <br>    
                                </div></cfif>---->
   ...
   <!----
    <cfif session.ep.USERID eq 325>
    <cfdump  var="#type_1_list#"> type_1_list<br>
    <cfdump  var="#type_2_list#"> type_2_list<br>
    </cfif>--->
        			...
        			<cfif get_shipping_clear.recordcount>
                        ...
                        <cfoutput query="get_shipping_clear">
                                <tr style="height:30px" <cfif get_shipping_clear.SEVK_EMP gt 0>class="color-row"<cfelse>class="color-light"</cfif>>
                                    ...
                                    <td style="text-align:center;<cfif DURUM neq 1>background-color:red</cfif>">
                                        <a href="javascript://" onclick="windowopen('#request.self#?fuseaction=objects.popup_print_files_old&print_type=32&action_id=#is_type#-#SHIP_RESULT_ID#','page');">
                        ...
                        <tr>
                            ...
                            <td colspan="10">
                            	...
                            	<input type="button" name="goster" value="Getir" onClick="arabenibulbeni()">
...
<script language="javascript">
	...
	function degisim(is_type,action_id)
	{
		...
		sevk_emp_id = document.getElementById('sevk_emp_id_'+action_id).value;
		  $.ajax({
                <cfoutput>url: '/cfc/shipping_op.cfc?method=upd_sevk_emp&sevk_emp_id='+sevk_emp_id+'&is_type='+is_type+'&action_id='+action_id+"&data_source=#dsn#",</cfoutput>
