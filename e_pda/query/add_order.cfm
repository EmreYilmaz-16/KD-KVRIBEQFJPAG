...
<cfif not isdefined("is_from_import") and not isdefined("add_relation_rows")>BURAYA GİRDİM DOLAYISIYLA AŞAĞIYADA GİRDİM</cfif>
...
<cflock name="#CREATEUUID()#" timeout="90">
	 <cftry> 
		...
		SİPARİŞ BAŞLIĞINI ALDIM KAYDETTİM LOOPU BAŞLATTIM
		<cfif isdefined('attributes.is_spect_name_to_property') and attributes.is_spect_name_to_property eq 1 and not isdefined("GetProductConf")>BURAYA GİRDİM DOLAYISIYLA AŞAĞIYADA GİRDİM</cfif>
	...
	<cfloop from="1" to="#attributes.rows_#" index="i">
		...
		<!-----<cfset get_max_order_row.order_row_id = 0> sİLİNECEK----->
		...
		ASORTİLER
		<cfinclude template="/V16/sales/query/add_assortment_textile_js.cfm">
		ASORTİ BİTİŞ
		<!--- //  urun asortileri --->
	</cfloop>
	LOOPU BİTİRDİM HAYIRLI OLASUN
	...
	<cfscript>
		...
		if(not isdefined("is_from_import") and not isdefined("add_reserve_row"))//importdan geliyorsa fonksiyon tanımlanmasın
			include('add_order_row_reserved_stock_PDA.cfm','\V16\objects\functions'); //rezerve edilen satırlar icin ORDER_ROW_RESERVED'a kayıt atıyor.
		add_reserve_row(
	...
	<!---<cfif isDefined("session.ep") and not isdefined("is_from_import")><!--- pda den gelen kayıtlar için --->
		<cf_workcube_process
			...
			action_page='#request.self#?fuseaction=#listgetat(attributes.fuseaction,1,'.')#.detail_order&order_id=#get_max_order.max_id#'
