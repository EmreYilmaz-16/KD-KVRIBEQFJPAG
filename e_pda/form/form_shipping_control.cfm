...
<cfquery name="getSerialNumbers" datasource="#DSN3#">
	SELECT ISNULL( (
	SELECT SERIAL_NO,STOCK_ID,(SELECT COUNT(*) FROM #dsn3#.PBS_SHIPPING_PACKAGE_LIST_SERIALS where SERIAL_NUMBER=SGN.SERIAL_NO) as IS_READ  FROM w3Qa_1.SERVICE_GUARANTY_NEW AS SGN
INNER JOIN (
	<cfloop query="getPeriods">
		SELECT FIS_ID,#getPeriods.PERIOD_ID# AS PERIOD_ID FROM #dsn#_#getPeriods.PERIOD_YEAR#_#OUR_COMPANY_ID#.STOCK_FIS WHERE REF_NO='#attributes.DELIVER_PAPER_NO#'
		<cfif getPeriods.currentrow LT getPeriods.recordcount>
			UNION ALL
		</cfif>
	</cfloop>  	
  ) AS SF ON SGN.PROCESS_ID=SF.FIS_ID AND SGN.PERIOD_ID=SF.PERIOD_ID
  WHERE SGN.PROCESS_CAT=113 AND IN_OUT=1
FOR JSON AUTO),'[]')  AS SERI_NUMARALI
</cfquery>
