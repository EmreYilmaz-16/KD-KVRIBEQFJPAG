           		...
           		WHERE      
                	SI.DISPATCH_SHIP_ID IN
                                            ...
                                            SELECT     
                                            	DISPATCH_SHIP_ID
											FROM         
                                            	#dsn2_alias#.EZGI_SHIP_INTERNAL_COLLECT
											WHERE     
                                            	COLLECT_ID = '#attributes.ship_collect_id#'
                                            )
             	GROUP BY 
...
{	
	...
	if(uzunluk > 9)
	{
		...
		var lot_sql = "SELECT PAKET_SAYISI FROM EZGI_SHIPPING_COLLECT_BARCOD WHERE COLLECT_ID ='"+ship_collect_id+"' AND BARCOD = '"+barcode+"'";
