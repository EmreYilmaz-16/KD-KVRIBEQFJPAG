...
var Config = {
	...
	QUERIES: {
		...
		STOCK_BY_SERIAL: "SELECT TOP 1 SB.STOCK_ID,SB.SERIAL_NO,PU.MAIN_UNIT,PU.MULTIPLIER,S.PRODUCT_NAME FROM "+dsn3+".SERVICE_GUARANTY_NEW AS SB INNER JOIN "+dsn3+".STOCKS AS S ON SB.STOCK_ID = S.STOCK_ID INNER JOIN "+dsn3+".PRODUCT_UNIT AS PU ON S.PRODUCT_UNIT_ID = PU.PRODUCT_UNIT_ID WHERE SB.SERIAL_NO = '{{serial}}'",
