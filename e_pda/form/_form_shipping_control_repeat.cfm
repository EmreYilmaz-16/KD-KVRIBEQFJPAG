...
<cfquery name="get_ship_package_list_repeat_info" datasource="#dsn3#">
	SELECT TOP (1) SHIP_METHOD_ID, ISNULL(IS_MAILED,0) AS IS_MAILED FROM EZGI_SHIPPING_PACKAGE_LIST_REPEAT WHERE SHIPPING_ID = #attributes.ship_id# AND TYPE = #attributes.is_type#
</cfquery>
<cfif get_ship_package_list_repeat_info.recordcount>
	<cfset attributes.ezgi_is_mailed = get_ship_package_list_repeat_info.IS_MAILED>
    <cfset attributes.ezgi_ship_method_id = get_ship_package_list_repeat_info.SHIP_METHOD_ID>
<cfelse>
	<cfset attributes.ezgi_is_mailed = 0>
    <cfset attributes.ezgi_ship_method_id = 0>
</cfif>
...
<form name="add_package" method="post" action="<cfoutput>#request.self#?fuseaction=pda.popup_add_shipping_package_repeat&SHIP_ID=#attributes.ship_id#&department_id=#attributes.department_id#&date1=#attributes.date1#&date2=#attributes.date2#&page=#attributes.page#&kontrol_status=#attributes.kontrol_status#&is_type=#attributes.is_type#</cfoutput>">
	<input type="hidden" value="<cfoutput>#attributes.ezgi_is_mailed#</cfoutput>" name="mail_gonder" />
	<tr class="color-list">
		<td colspan="5">
		<table cellpadding="2" cellspacing="1" width="75%">
			<tr height="20px">
            	<td colspan="4">
                    ...
                    <b>Mail:</b><input disabled="disabled" type="checkbox" name="is_mailed" <cfif attributes.ezgi_is_mailed eq 1>checked</cfif>
                </td>
           	</tr>
            <tr height="20px">
            	<td colspan="4">
					<select name="ezgi_ship_method_id" style="width:240px">
                    	<option value="" <cfif attributes.ezgi_ship_method_id eq 0 or attributes.ezgi_ship_method_id eq ''>selected</cfif>>Seçiniz</option>
                    	<cfoutput query="get_ship_method">
                    		<option value="#ship_method_id#" <cfif attributes.ezgi_ship_method_id eq ship_method_id>selected</cfif>>#ship_method#</option>
                        </cfoutput>
                    </select>
                </td>
           	</tr>
...
{	
	...
	if(uzunluk > 9)
	{
		...
		var lot_sql = "SELECT TOP (1) SHIP_RESULT_ID FROM EZGI_SHIPPING_BARCOD WHERE SHIP_RESULT_ID ="+ship_id+"  AND IS_TYPE ="+is_type+"  AND LOT_NO = '"+lot_number+"' AND BARCOD = '"+barcode+"'";
