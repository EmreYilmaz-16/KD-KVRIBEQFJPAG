...
<script language="javascript" type="text/javascript">
	...
	function kontrol_kayit()
	{
			...
			document.getElementById('onay').disabled = true;
			actionidolustur();
			window.location.href='<cfoutput>#request.self#</cfoutput>?fuseaction=pda.add_ambar_fis_wb&dep_in='+form_basket.txt_department_in.value+'&dep_out='+form_basket.txt_department_out.value+'&action_id='+document.getElementById('action_id').value+'&fis_tipi='+form_basket.fis_tipi.value+'&process_cat='+form_basket.process_cat_id.value;
			}
	}
</script>
<script>
