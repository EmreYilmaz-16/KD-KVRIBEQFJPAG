...
WHERE SR.SHIP_ID=#attributes.shipId#
 ) AS TFR
 ORDER BY PRODUCT_ID ------>
...
FROM (
	SELECT S.STOCK_ID
		...
		,(
			...
			FROM (
				SELECT COUNT(*) T
				FROM #dsn3#.SERVICE_GUARANTY_NEW AS SGA
				WHERE SGA.WRK_ROW_ID = SR.WRK_ROW_ID
				
				UNION ALL
				
				SELECT COUNT(*) T
				FROM w3Qa_1.PBS_MAL_KABUL_BARCODES AS SGA
				WHERE SGA.WRK_ROW_ID = SR.WRK_ROW_ID
				) AS TKS
			) AS OMIK
		,(
