...
SELECT ORR.WRK_ROW_ID
    ...
    ,ISNULL(MARJ_ORAN_PBS,0) AS MARJ_ORAN_PBS
    ,ISNULL(PRICE_PBS,PRICE) AS PRICE_PBS
	,ORR.DETAIL_INFO_EXTRA AS OEM_NUMARA
	,ORR.PRODUCT_NAME
    ,P.PRODUCT_CODE
	,PC.PRODUCT_CAT
	,ISNULL(PB.BRAND_NAME, 'MARKA YOK') BRAND_NAME
	,ISNULL(PBM.MODEL_NAME, 'MODEL YOK') MODEL_NAME
	,PBM.MODEL_ID
FROM #dsn3#.OFFER_ROW AS ORR
LEFT JOIN #dsn1#.PRODUCT AS P ON P.PRODUCT_ID = ORR.PRODUCT_ID
LEFT JOIN #dsn1#.STOCKS AS PS ON PS.STOCK_ID = ORR.STOCK_ID
LEFT JOIN #dsn1#.PRODUCT_CAT AS PC ON PC.PRODUCT_CATID = P.PRODUCT_CATID
LEFT JOIN #dsn1#.PRODUCT_BRANDS AS PB ON PB.BRAND_ID = P.BRAND_ID
LEFT JOIN #dsn1#.PRODUCT_BRANDS_MODEL AS PBM ON PBM.MODEL_ID = P.SHORT_CODE_ID
