...
<cfquery name="getofferrow" datasource="#dsn3#">
    SELECT ORR.WRK_ROW_ID,ORR.PRODUCT_ID,ORR.PRICE,ORR.PRICE_OTHER,ORR.QUANTITY,ORR.DISCOUNT_1,ORR.DISCOUNT_2,ORR.DISCOUNT_3,(SELECT TOP 1 BARCODE FROM w3Qa_product.STOCKS_BARCODES AS S WHERE STOCK_ID=P.STOCK_ID) AS OEM_NUMARA,ORR.PRODUCT_NAME,PC.PRODUCT_CAT,ISNULL(PB.BRAND_NAME,'MARKA YOK') BRAND_NAME,ISNULL(PBM.MODEL_NAME,'MODEL YOK') MODEL_NAME,PBM.MODEL_ID FROM w3Qa_1.OFFER_ROW AS ORR
LEFT JOIN w3Qa_product.STOCKS AS P ON P.PRODUCT_ID=ORR.PRODUCT_ID
