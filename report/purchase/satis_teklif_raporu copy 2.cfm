...
SELECT DISTINCT   SATIS_TEKLIFI.OFFER_NUMBER AS SATIS_TEKLIF_NO, 
    ...
    INVOICE.INVOICE_ID AS SATIS_FATURA_ID,
    INVOICE.INVOICE_DATE AS SATIS_FATURA_TARIHI,
    INVOICE.PERIOD_ID AS SATIS_FATURA_PERIOD  FROM #dsn3#.INTERNALDEMAND_ROW AS IRR
LEFT JOIN #dsn3#.INTERNALDEMAND AS IDD ON IDD.INTERNAL_ID=IRR.I_ID
LEFT JOIN #dsn3#.OFFER_ROW AS ALIS_TEKLIFI_SATIRLARI_MAIN ON ALIS_TEKLIFI_SATIRLARI_MAIN.WRK_ROW_RELATION_ID=IRR.WRK_ROW_ID
LEFT JOIN #dsn3#.OFFER_ROW AS ALIS_TEKLIFI_SATIRLARI ON ALIS_TEKLIFI_SATIRLARI.WRK_ROW_RELATION_ID=ALIS_TEKLIFI_SATIRLARI_MAIN.WRK_ROW_ID
LEFT JOIN #dsn3#.OFFER AS ALIS_TEKLIFI ON ALIS_TEKLIFI.OFFER_ID=ALIS_TEKLIFI_SATIRLARI.OFFER_ID
LEFT JOIN #dsn3#.OFFER_ROW AS SATIS_TEKLIFI_SATIRLARI ON SATIS_TEKLIFI_SATIRLARI.WRK_ROW_RELATION_ID=ALIS_TEKLIFI_SATIRLARI.WRK_ROW_ID
LEFT JOIN #dsn3#.OFFER  AS SATIS_TEKLIFI ON SATIS_TEKLIFI.OFFER_ID=SATIS_TEKLIFI_SATIRLARI.OFFER_ID
 LEFT JOIN #dsn3#.ORDER_ROW AS SATIS_SIPARISI_SATIRLARI ON SATIS_SIPARISI_SATIRLARI.WRK_ROW_RELATION_ID = SATIS_TEKLIFI_SATIRLARI.WRK_ROW_ID
    LEFT JOIN #dsn3#.ORDERS AS SATIS_SIPARISI ON SATIS_SIPARISI.ORDER_ID = SATIS_SIPARISI_SATIRLARI.ORDER_ID
    LEFT JOIN #dsn3#.ORDER_ROW AS ALIS_SIPARIS_SATIRLARI 
        ON ALIS_SIPARIS_SATIRLARI.WRK_ROW_RELATION_ID = CAST(SATIS_TEKLIFI_SATIRLARI.WRK_ROW_ID AS VARCHAR) + '_XX'
