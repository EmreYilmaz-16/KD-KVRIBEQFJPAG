...
FROM #dsn3#.INTERNALDEMAND_ROW AS IRR
LEFT JOIN #dsn3#.INTERNALDEMAND AS IDD 
    ON IDD.INTERNAL_ID = IRR.I_ID
LEFT JOIN #dsn3#.OFFER_ROW AS ALIS_TEKLIFI_SATIRLARI_MAIN 
    ON ALIS_TEKLIFI_SATIRLARI_MAIN.WRK_ROW_RELATION_ID = IRR.WRK_ROW_ID
LEFT JOIN #dsn3#.OFFER_ROW AS ALIS_TEKLIFI_SATIRLARI 
    ON ALIS_TEKLIFI_SATIRLARI.WRK_ROW_RELATION_ID = ALIS_TEKLIFI_SATIRLARI_MAIN.WRK_ROW_ID
LEFT JOIN #dsn3#.OFFER AS ALIS_TEKLIFI 
    ON ALIS_TEKLIFI.OFFER_ID = ALIS_TEKLIFI_SATIRLARI.OFFER_ID
LEFT JOIN #dsn3#.OFFER_ROW AS SATIS_TEKLIFI_SATIRLARI 
    ON SATIS_TEKLIFI_SATIRLARI.WRK_ROW_RELATION_ID = ALIS_TEKLIFI_SATIRLARI.WRK_ROW_ID
LEFT JOIN #dsn3#.OFFER AS SATIS_TEKLIFI 
    ON SATIS_TEKLIFI.OFFER_ID = SATIS_TEKLIFI_SATIRLARI.OFFER_ID AND SATIS_TEKLIFI.OFFER_STAGE<>267 <!--- TODO: Buraya dikkat et böyle bir süreç yok  --->
...
LEFT JOIN #dsn3#.OFFER_ROW AS SATIS_TEKLIFI_SATIRLARI 
    ON SATIS_TEKLIFI_SATIRLARI.WRK_ROW_RELATION_ID = ALIS_TEKLIFI_SATIRLARI.WRK_ROW_ID
LEFT JOIN #dsn3#.ORDER_ROW AS SATIS_SIPARISI_SATIRLARI 
    ON SATIS_SIPARISI_SATIRLARI.WRK_ROW_RELATION_ID = SATIS_TEKLIFI_SATIRLARI.WRK_ROW_ID
LEFT JOIN #dsn3#.ORDERS AS SATIS_SIPARISI 
...
SELECT DISTINCT 
    ...
    INVOICE2.INVOICE_ID AS ALIS_FATURA_ID,
...
LEFT JOIN #dsn3#.ORDERS AS ALIS_SIPARIS 
    ON ALIS_SIPARIS.ORDER_ID = ALIS_SIPARIS_SATIRLARI.ORDER_ID
LEFT JOIN #dsn3#.ORDERS_INVOICE AS OI2 
...
SELECT DISTINCT 
    ...
    INVOICE2.SHIP_ID AS ALIS_IRSALIYE_ID
...
LEFT JOIN #dsn3#.ORDERS AS ALIS_SIPARIS 
    ON ALIS_SIPARIS.ORDER_ID = ALIS_SIPARIS_SATIRLARI.ORDER_ID
LEFT JOIN #dsn3#.ORDERS_SHIP AS OI2 
    ON OI2.ORDER_ID = ALIS_SIPARIS.ORDER_ID
LEFT JOIN (
    SELECT SHIP_NUMBER, SHIP_DATE, SHIP_ID, 1 AS PERIOD_ID FROM #dsn#_#year(now())-1#_#session.ep.company_id#.SHIP
    UNION ALL
    SELECT SHIP_NUMBER, SHIP_DATE, SHIP_ID,2 AS PERIOD_ID FROM #dsn2#.SHIP
) AS INVOICE2 
...
SELECT DISTINCT 
    ...
    INVOICE.SHIP_ID AS SATIS_IRSALIYE_ID
...
SELECT DISTINCT 
    ...
    SATIS_FATURA.SATIS_FATURA_ID AS SATIS_FATURA_ID,
    ...
    SATIS_IRSALIYE.SATIS_IRSALIYE_ID AS SATIS_IRSALIYE_ID


        
FROM #dsn3#.INTERNALDEMAND AS IDO
    LEFT JOIN #dsn3#.OFFER AS OFFER_MAIN 
        ON OFFER_MAIN.INTERNALDEMAND_ID = IDO.INTERNAL_ID
    LEFT JOIN #dsn3#.OFFER AS OFFER_SUB 
        ON OFFER_SUB.FOR_OFFER_ID = OFFER_MAIN.OFFER_ID
    LEFT JOIN #dsn#.COMPANY AS C 
        ON C.COMPANY_ID = IDO.FROM_COMPANY_ID
    LEFT JOIN #dsn#.EMPLOYEE_POSITIONS AS EP 
        ON EP.POSITION_CODE = IDO.TO_POSITION_CODE
    LEFT JOIN #dsn#.COMPANY AS C2 
    ...
    LEFT JOIN  TEST00000 O ON O.OFFER_ID=OFFER_SUB.OFFER_ID
    --       OUTER APPLY (
    --     SELECT ORDER_NUMBER,ORDER_ID FROM #dsn3#.ORDERS WHERE ORDER_ID IN (
    --     SELECT ORDER_ID FROM #dsn3#.ORDER_ROW AS ORR WHERE ORR.WRK_ROW_RELATION_ID IN(
    --         SELECT WRK_ROW_ID+'_XX' FROM #dsn3#.OFFER_ROW WHERE OFFER_ID=SATIS_TEKLIF.SATIS_TEKLIF_ID
    --     ))
    -- ) AS O
...
data.forEach(item => {
  ...
  if (item.ALT_TEKLIF_NO) {
    grouped[key].teklifler[anaTeklifKey].altTeklifler.push({
      ...
      satisIrsaliyeId: item.SATIS_IRSALIYE_ID,
      ...
      satislarUnique.forEach(sat => {
        block.innerHTML += `
          <li>
            ...
            ${sat.satisIrsaliyeNo ? `<a href='/index.cfm?fuseaction=stock.form_add_sale&event=upd&ship_id=${sat.satisIrsaliyeId}'><span class="tarih">📦 ${sat.satisIrsaliyeNo}</span></a>` : ""}
