...
if(ShowType=="0"){
    ...
    satis.ALISLAR.forEach(alis => {
     ...
     html += `<div class="sub-section" style="display:flex">`;
      ...
      if (alis.SIPARIS_NO) {
        html += ` <hr style="border: 0.1px solid gray;width: 100%;"> <span class="text-success">📬 Sipariş: </span> <a href="javascript://" onclick='window.location.href="/index.cfm?fuseaction=purchase.list_order&event=upd&order_id=${alis.SIPARIS_ID}"'>${alis.SIPARIS_NO} <small class="text-muted">📅 ${new Date(alis.SIPARIS_TARIH).toLocaleDateString()}</small></a>`;
      }
      html += `</div>`;
    });
}
