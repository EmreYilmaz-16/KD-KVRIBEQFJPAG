...
<cfquery name="getPaperSerials" datasource="#dsn3#">
    SELECT (
        SELECT DISTINCT
            ...
            CASE WHEN ISNULL(SG.SERIAL_NO,'')='' THEN 0 ELSE 1 END AS IS_SERIAL
        ...
        LEFT JOIN (
           <!---- SELECT FIS_ID, REF_NO, 2 AS PERIODID FROM #dsn2#.STOCK_FIS
            ...
            <cfloop query="GETPERIODS">
                ...
                SELECT SF#PERIOD_YEAR#.FIS_ID, REF_NO, #GETPERIODS.PERIOD_ID# AS PERIODID,STOCK_ID FROM #dsn#_#GETPERIODS.PERIOD_YEAR#_#GETPERIODS.OUR_COMPANY_ID#.STOCK_FIS AS SF#PERIOD_YEAR#
        ...
        LEFT JOIN #dsn3#.STOCKS S ON S.STOCK_ID = SF.STOCK_ID
