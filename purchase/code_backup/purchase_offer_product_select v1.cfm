...
<CFSET OFFER_STAGE="0">
  <cfquery name="getrelofferID" datasource="#DSN3#">
    SELECT * FROM #dsn3#.PURCHAE_OFFER_SALE_OFFER_RELATION_PBS WHERE  PURCHASE_OFFER_ID= #attributes.offer_id#
  </cfquery>
  <CFIF getrelofferID.recordCount>
    <cfquery name="GETRELATEDOFFER" datasource="#DSN3#">
      SELECT * FROM #dsn3#.OFFER WHERE  OFFER_ID = #getrelofferID.SALE_OFFER_ID#
    </cfquery>
...
SELECT 
    ...
    (
        SELECT 
            ...
            CASE WHEN (SELECT COUNT(*) FROM #dsn3#.PBS_SELECTED_ROWS WHERE WRK_ROW_ID=OFFER_ROW.WRK_ROW_ID)>0 THEN 1 ELSE 0 END AS IS_SELECTED,

            CASE WHEN (SELECT COUNT(*) FROM #dsn3#.OFFER_ROW AS TTTTTTT WHERE WRK_ROW_RELATION_ID=OFFER_ROW.WRK_ROW_ID)>0 THEN 1 ELSE 0 END AS IS_SATINALMA
            ,ISNULL((SELECT CAST(PRODUCT_MARJ AS DECIMAL(18,2)) AS PRODUCT_MARJ,CAST(SALE_PRICE AS DECIMAL(18,2)) AS SALE_PRICE  FROM #dsn3#.PBS_SELECTED_ROWS WHERE WRK_ROW_ID=OFFER_ROW.WRK_ROW_ID FOR JSON AUTO),'[]') AS SLP
            ,ISNULL((
				SELECT * FROM (
				SELECT PRODUCT_ID FROM #dsn3#.ALTERNATIVE_PRODUCTS WHERE PRODUCT_ID=OFFER_ROW.PRODUCT_ID OR ALTERNATIVE_PRODUCT_ID=OFFER_ROW.PRODUCT_ID
				UNION ALL
				SELECT ALTERNATIVE_PRODUCT_ID PRODUCT_ID FROM #dsn3#.ALTERNATIVE_PRODUCTS WHERE PRODUCT_ID=OFFER_ROW.PRODUCT_ID OR ALTERNATIVE_PRODUCT_ID=OFFER_ROW.PRODUCT_ID
				) AS TABLO
