...
FROM #dsn3#.OFFER_ROW ORR
LEFT JOIN #dsn3#.STOCKS S ON S.STOCK_ID = ORR.STOCK_ID
LEFT JOIN #dsn3#.PRODUCT_CAT PC ON PC.PRODUCT_CATID = S.PRODUCT_CATID
LEFT JOIN #dsn3#.PRODUCT_BRANDS PB ON PB.BRAND_ID = S.BRAND_ID
LEFT JOIN #dsn1#.PRODUCT_BRANDS_MODEL PBM ON PBM.MODEL_ID = S.SHORT_CODE_ID

LEFT JOIN (
    SELECT PRODUCT_ID, ALTERNATIVE_PRODUCT_ID FROM #dsn3#.ALTERNATIVE_PRODUCTS    
) AS AP ON AP.PRODUCT_ID = ORR.PRODUCT_ID

LEFT JOIN #dsn3#.OFFER_ROW ALT_ORR ON ALT_ORR.PRODUCT_ID = AP.ALTERNATIVE_PRODUCT_ID
    AND ALT_ORR.OFFER_ID = ORR.OFFER_ID
    LEFT JOIN #dsn3#.STOCKS ALT_S ON ALT_S.STOCK_ID = ALT_ORR.STOCK_ID
LEFT JOIN #dsn3#.PRODUCT_CAT ALT_PC ON ALT_PC.PRODUCT_CATID = ALT_S.PRODUCT_CATID
LEFT JOIN #dsn3#.PRODUCT_BRANDS ALT_PB ON ALT_PB.BRAND_ID = ALT_S.BRAND_ID
LEFT JOIN #dsn1#.PRODUCT_BRANDS_MODEL ALT_PBM ON ALT_PBM.MODEL_ID = ALT_S.SHORT_CODE_ID
...
FROM 
    #dsn3#.OFFER_ROW AS ORR
LEFT JOIN 
    #dsn3#.STOCKS AS S ON S.STOCK_ID = ORR.STOCK_ID
LEFT JOIN 
    #dsn3#.PRODUCT_CAT AS PC ON PC.PRODUCT_CATID = S.PRODUCT_CATID
LEFT JOIN 
    #dsn3#.PRODUCT_BRANDS AS PB ON PB.BRAND_ID = S.BRAND_ID
LEFT JOIN 
    #dsn1#.PRODUCT_BRANDS_MODEL AS PBM ON PBM.MODEL_ID = S.SHORT_CODE_ID
WHERE 
    ORR.OFFER_ID = 81;
</cfquery>
