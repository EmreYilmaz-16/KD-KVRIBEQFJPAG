...
,CAST(PRICE AS DECIMAL(18, 2)) AS PRICE
  ...
  ,CASE WHEN (SELECT TOP 1 (SELECT COUNT(*) FROM w3Qa_1.ORDER_ROW WHERE WRK_ROW_RELATION_ID=OFR2.WRK_ROW_ID+'_XX') FROM w3Qa_1.OFFER_ROW OFR2 WHERE WRK_ROW_RELATION_ID=OFFER_ROW.WRK_ROW_ID) >0	THEN 1 ELSE 0 END AS SNT_S
	, CASE 
		WHEN (
				SELECT COUNT(*)
