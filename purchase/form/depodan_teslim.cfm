...
,CAST(PRICE AS DECIMAL(18, 2)) AS PRICE
  ...
  ,dbo.fn_IsUserAllowed_PBS(#session.ep.userid#, N'purchase', 1, OFFER_ROW.PRODUCT_ID, NULL,NULL) AS IS_ALLOWED
	, OFFER_ROW.STOCK_ID
  ,OFFER_ROW.DETAIL_INFO_EXTRA  AS OEM_NO 
  ,ISNULL((SELECT TOP 1 PRICE FROM #DSN#.CMP_PRICE_ALL WHERE STOCK_ID=OFFER_ROW.STOCK_ID AND COMPANY_ID=#GETDEMAND_MONEY.FROM_COMPANY_ID# AND PRICE>0 ORDER BY INVOICE_DATE DESC),0) AS LAST_PRICE
	, WRK_ROW_ID
