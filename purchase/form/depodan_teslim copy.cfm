...
function getConvertedNetPriceWithMarj(productName, marj = 0) {
    let net = 0;
...
function calculateFinalSalePrice(productName) {
...
if (data.length === 1) {
    ...
    salePriceInput.value = getConvertedNetPriceWithMarj(productName, marj);
} else {
...
marjInput.addEventListener('input', () => {
    calculateFinalSalePrice(productName);
});
