...
UNION ALL
SELECT PRICE_OTHER,INVOICE_ROW.OTHER_MONEY,INVOICE.INVOICE_DATE,STOCK_ID,COMPANY_ID FROM #dsn#_#year(now())-1#_#session.ep.company_id#
INNER JOIN #dsn#_#year(now())-1#_#session.ep.company_id#NVOICE_ID
WHERE INVOICE.PURCHASE_SALES=1 --AND STOCK_ID=75 AND COMPANY_ID=9
) AS T
...
,CAST(PRICE AS DECIMAL(18, 2)) AS PRICE
  ...
  ,CASE WHEN (SELECT (SELECT COUNT(*) FROM #dsn3#.ORDER_ROW WHERE WRK_ROW_RELATION_ID=OFR2.WRK_ROW_ID+'_XX') FROM #dsn3#.OFFER_ROW OFR2 WHERE WRK_ROW_RELATION_ID=OFFER_ROW.WRK_ROW_ID) >0	THEN 1 ELSE 0 END AS SNT_S
	, CASE 
		WHEN (
				SELECT COUNT(*)
