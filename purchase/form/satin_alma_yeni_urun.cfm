...
(SELECT MONEY_TYPE,CONVERT(DECIMAL(18,6),RATE2) RATE2 FROM #dsn3#.OFFER_MONEY WHERE ACTION_ID=OFFER_ID FOR JSON PATH) AS OFFER_MONEY_ARR,
(
...
WHERE OFFER_ID = O.OFFER_ID AND ISNULL(OFFER_ROW.SELECT_INFO_EXTRA,0) NOT IN (3,4,5,6)
FOR JSON PATH
) AS URUNLER
