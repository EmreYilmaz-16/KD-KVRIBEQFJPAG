...
(SELECT MONEY_TYPE,CONVERT(DECIMAL(18,6),RATE2) RATE2 FROM w3Qa_1.OFFER_MONEY WHERE ACTION_ID=OFFER_ID FOR JSON PATH) AS OFFER_MONEY_ARR,
(
...
,CAST(PRICE AS DECIMAL(18, 2)) AS PRICE
  ...
  ,CASE WHEN (SELECT TOP 1 (SELECT COUNT(*) FROM w3Qa_1.ORDER_ROW WHERE WRK_ROW_RELATION_ID=OFR2.WRK_ROW_ID+'_XX') FROM w3Qa_1.OFFER_ROW OFR2 WHERE WRK_ROW_RELATION_ID=OFFER_ROW.WRK_ROW_ID <cfif isDefined("attributes.last_offer_id") and len(attributes.last_offer_id)>AND OFFER_ID=#attributes.last_offer_id#</cfif>) >0	THEN 1 ELSE 0 END AS SNT_S
...
WHERE OFFER_ID = O.OFFER_ID AND ISNULL(OFFER_ROW.SELECT_INFO_EXTRA,0) NOT IN (3,4)
FOR JSON PATH
) AS URUNLER
