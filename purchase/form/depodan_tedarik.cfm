...
,CAST(PRICE AS DECIMAL(18, 2)) AS PRICE
  ...
  ,CAST(PRICE - (PRICE * DISCOUNT_1 / 100) AS DECIMAL(18, 2)) AS ONET_PRICE_2
	, CAST(ISNULL(GPA_PRICE,-1) AS DECIMAL(18,2)) AS  NET_PRICEXX --, CAST(PRICE - (PRICE * DISCOUNT_1 / 100) AS DECIMAL(18, 2)) AS NET_PRICE
   ,CASE WHEN (SELECT TOP 1 (SELECT COUNT(*) FROM w3Qa_1.ORDER_ROW WHERE WRK_ROW_RELATION_ID=OFR2.WRK_ROW_ID+'_XX') FROM w3Qa_1.OFFER_ROW OFR2 WHERE WRK_ROW_RELATION_ID=OFFER_ROW.WRK_ROW_ID <cfif len(attributes.last_offer_id)>AND OFFER_ID=#attributes.last_offer_id#</cfif>) >0	THEN 1 ELSE 0 END AS SNT_S
...
LEFT JOIN w3Qa_product.PRODUCT_BRANDS  AS PB ON PB.BRAND_ID =S.BRAND_ID
LEFT JOIN
            (
                SELECT
                    P.UNIT									GPA_UNIT,
                    ...
                    w3qa_1.GET_DISCOUNT_RATE(#GETDEMAND_MONEY.FROM_COMPANY_ID#,#CMPRICE_ID#,P.PRODUCT_ID) AS DSC_OX,
                    w3qa_1.GET_DISCOUNT_RATE2(#GETDEMAND_MONEY.FROM_COMPANY_ID#,#CMPRICE_ID#,P.PRODUCT_ID) AS DSC_OX2,
                    'ODL='+CAST(w3qa_1.GET_DISCOUNT_RATE(TRY_CAST(REPLACE(O.OFFER_TO, ',', '') AS INT),P.PRICE_CATID,P.PRODUCT_ID) AS VARCHAR)+'|PRC='+CAST(P.PRICE AS VARCHAR)+'|DSC='+CAST(P.PRICE_DISCOUNT AS VARCHAR)+'|CAT='+CAST(P.PRICE_CATID AS VARCHAR)+'|CID='+CAST(TRY_CAST(REPLACE(O.OFFER_TO, ',', '') AS INT) AS VARCHAR)+'|PID='+CAST(P.PRODUCT_ID AS VARCHAR) AS ODL_INFO,
                    'ODL_2='+CAST(w3qa_1.GET_DISCOUNT_RATE(#GETDEMAND_MONEY.FROM_COMPANY_ID#,#CMPRICE_ID#,P.PRODUCT_ID) AS VARCHAR)+'|PRC='+CAST(P.PRICE AS VARCHAR)+'|DSC='+CAST(P.PRICE_DISCOUNT AS VARCHAR)+'|CAT='+CAST(P.PRICE_CATID AS VARCHAR)+'|CID='+CAST(#GETDEMAND_MONEY.FROM_COMPANY_ID# AS VARCHAR)+'|PID='+CAST(P.PRODUCT_ID AS VARCHAR) AS ODL_INFO_2,
                    (			SELECT 
             ...
             FROM w3Qa.SETUP_MONEY AS SM WHERE SM.PERIOD_ID=#session.ep.PERIOD_ID#
